...
void alexa_set_ext_mode(enum alexa_ext_mode mode)
{
    pthread_mutex_lock(&ev_mutex);
    alexa_current_ext_mode = mode;
    pthread_mutex_unlock(&ev_mutex);
}

enum alexa_ext_mode alexa_get_ext_mode()
{
    return alexa_current_ext_mode;
}

bool isWifiMode()
{
    return (alexa_current_mode == ALEXA_WIFI_MODE);
}
...
int event_process(void)
{
    ...
    while (0 == ev_unprocessed) {
        printf("event_process pthread_cond_wait before.\n");
        pthread_cond_wait(&ev_pending, &ev_mutex);
        printf("event_process pthread_cond_wait end. alexa_current_mode:%d; alexa_current_ext_mode:%d.\n", alexa_current_mode, alexa_current_ext_mode);

        switch (alexa_current_ext_mode) {
        case ALEXA_EXT_MODE_NORMAL:
            alexa_current_ext_mode = ALEXA_EXT_MODE_NONE;
            if (alexa_current_mode == ALEXA_NORMAL_MODE) {
                ev_unprocessed = 0;
                break;
            } else {
                alexa_current_mode = ALEXA_NORMAL_MODE;
                pthread_mutex_unlock(&ev_mutex);
                return FUNC_KEY_NORMAL_MODE;
            }
        case ALEXA_EXT_MODE_WIFI:
            alexa_current_ext_mode = ALEXA_EXT_MODE_NONE;
            if (alexa_current_mode == ALEXA_WIFI_MODE) {
                ev_unprocessed = 0;
                break;
                ...
                alexa_current_mode = ALEXA_WIFI_MODE;
                pthread_mutex_unlock(&ev_mutex);
                return FUNC_KEY_WIFI_MODE;
            }
        case ALEXA_EXT_MODE_BT:
            alexa_current_ext_mode = ALEXA_EXT_MODE_NONE;
            if (alexa_current_mode == ALEXA_BT_MODE) {
                ev_unprocessed = 0;
                break;
    ...
    while(1) {
        ...
        if (ret == 0) {
            ...
            if (ev_signaled) {
                ...
                if (alexa_current_ext_mode != ALEXA_EXT_MODE_NONE) {
                    ev_signaled = 0;
                    pthread_cond_signal(&ev_pending);
                    ev_unprocessed = 1;
                }
...
int start_event_thread(void)
{
    ...
    return pthread_create(&evt_reader, NULL, &event_read_thread, NULL);
}
